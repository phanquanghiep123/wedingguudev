...
class themesys extends MY_Controller {
    public $_fix   = "theme_";
    public $_table = "themes";
    public $_view  = "backend/themes";
    public $_cname = "themesys";
    ...
    public function index(){
        ...
        $where = ["theme_themes.is_system" => 1,"version" => 3];
        if($this->input->get("keyword")){
            $where["theme_themes.name like"] = "%".$this->input->get("keyword")."%";
        }
        $this->_data["tables"] = $this->{$this->_model}->getsys($offset,$limit,$where);
        ...
        $this->_data["action_create"] = backend_url('themes/themes/create');
    ...
    public function clone ($id){
        ...
        $block_action_inserts = $section_setting_insert = $block_setting_inserts = $action_inserts = $sbods =  $section_action_insert = $style_meta_inserts = $old_ord_inserts =  $inserts = [];
        if(true){
            $this->load->model($this->_model);
            $this->load->model('Sections_model');
            $this->load->model('Blocks_model');
            ...
            foreach ($lsections as $key_s => $value) {
                ...
                $value['blocks']       = [];
                $theme_section_id = $value['theme_section_id'];
                $metas  = $this->Common_model->get_result_for_select($this->_fix.'block_part_meta','id,theme_section_id,section_block_id,block_part_id,meta_key,value,media_id,sort,allow_width',
                    [
                        'theme_section_id' => $theme_section_id
                    ],
                ...
                if($allScreenSecsion){
                    foreach ($allScreenSecsion as $key_allScreenSecsion => $value_allScreenSecsion) {
                       ...
                       $value_allScreenSecsion['version']  = $v;
                ...
                foreach ($lblocks as $key_b => $value_b) {
                    ...
                    $is_default = 0;
                    $value_b['section_block_id'] = $this->Common_model->add($this->_fix."section_block",[
                        "section_id" => $value_b['section_id'],
                        "block_id"   => $value_b['block_id'],
                        "clone_id"   => $section_block_id,
                        'version'    => $v,
                        "is_default" => 1
                    ]);
                    ...
                    if($section_block_order_settings){
                        foreach ($section_block_order_settings as $key_section_block_order_settings => $value_section_block_order_settings) {
                            ...
                            $value_section_block_order_settings["section_block_id"] = $value_b["section_block_id"];
                            $value_section_block_order_settings["version"] = $v;
                            $block_setting_inserts [] = $value_section_block_order_settings;
                        }
                    }
                    $sbod = $this->Common_model->get_recode_for_select($this->_fix.'section_block_order','section_block_id,ncolum,class_name,is_form',['section_block_id' => $section_block_id ,'theme_section_id' => $theme_section_id]);
                    if($sbod){
                        $sbod['theme_section_id'] = $value['theme_section_id'];
                        $sbod['section_block_id'] = $value_b['section_block_id'];
                        $sbod['is_default']       = 0;
                        $sbod['sort']             = $key_b;
                        $sbod['version']          = $v;
                        $sbods[]                  = $sbod;
                        ...
                        foreach ($lparts as $key_p => $value_p) {
                            ...
                            $value_p['block_part_id'] = $this->Common_model->add($this->_fix."section_block_part",
                                [
                                    "part_id"    => $value_p["part_id"],
                                    "block_id"   => $value_b['block_id'],
                                    "is_default" => 1,
                                    "name"       => $value_p["name"],
                                    'version'    => $v
                            ...
                            foreach ($metas as $key_m => $value_m) {
                                if($value_m['block_part_id'] == $block_part_id && $value_m['section_block_id'] == $section_block_id){
                                    $this->Common_model->add($this->_fix.'block_part_meta', [
                                        ...
                                        'allow_width'      => $value_m['allow_width'],
                                        'version'          => $v
                                    ]);
                                }   
                            }                                                               
                        ...
                        //!clone part block;
                        //clone action block;
                        $actions = $this->Blocks_model->get_actions($section_block_id,$theme_section_id,1);
                        foreach ($actions as $key => $value_a) {
            ...
            redirect(backend_url('themes/themes/edit/'.$theme_id));
        }
        return true; 
    }
        ...
        foreach ($filesfolder as $key => $file) {
            if(trim($file) != "." && trim($file) != ".." &&file_exists(FCPATH . $url . "/" .$file)){
                ...
                if(is_dir (FCPATH . $i["path"]) ){
                    ...
                    $dir_folder1  = str_replace("//", "/",$dir_folder1);
    ...
    public function medias_megge(){
        //$v1 = $this->Common_model->get_result($this->_fix.'medias');
        $v2 = $this->Common_model->get_result($this->_fix.'medias_v3');
        foreach ($v2 as $key => $value) {
            $id = $value["id"];
            unset( $value["id"]);
            $c = $this->Common_model->get_record($this->_fix.'medias',["id" => $id]);
            if($c){       
                $this->Common_model->update($this->_fix.'medias',$value,["id" => $id]);
            }else{
                $this->Common_model->add($this->_fix.'medias',$value);
            }
        }
        echo "<pre>";
        print_r( $v2);
