...
class Options extends MY_Controller {
    private $folder_view = "options"; 
    private $base_controller;
    private $table = '';
    public function __construct() {
        parent::__construct();
        $this->table = $this->table_prefix.'package_options';
    ...
    public function items(){
        ...
        $per_page = $this->per_page;
        $count_table = $this->Common_model->count_table("package_options",$where);
        $offset = ($this->input->get("offset") != "") ? $this->input->get("offset") : 0 ;    
        $config['base_url'] = base_url('/backend/'.$this->folder_view.'/'.$request);
        $config['total_rows'] = $count_table;
        $config['per_page'] = $per_page;
        ...
        $this->load->library('pagination');
        $this->pagination->initialize(_get_paging($config));
        $this->data["package"] = $this->Common_model->get_result("package_options",$where,$offset,$per_page);
        $this->load->view($this->backend_asset."/".$this->folder_view."/items",$this->data);
    }
}
