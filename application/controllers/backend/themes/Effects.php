...
class Effects extends MY_Controller {
  public $_fix   = "theme_";
	public $_table = "background_music";
	public $_view  = "backend/themes/effects";
  public $_cname = "themes/effects";
  ...
  public $_data  = [];
	...
	public function index (){
		...
		$this->_data["tables"] = $this->Common_model->get_result($this->_fix.$this->_table,["type" => 2],$offset,$limit);
		$total_rows = $this->Common_model->count_table($this->_fix.$this->_table,["type" => 2]);
	...
	public function delete($id){
		...
		public function autoinsert(){
			$id = '1497';
			$m = $this->Common_model->get_result("theme_medias",["folder_id" => $id]);
		
			$data = [];
			foreach ($m as $key => $value) {
				$name = $key + 1;
				if(strlen($name) == 1) $name = "00".$name;
			...
			print_r($data);
			echo "</pre>";
			$this->Common_model->insert_batch_data("theme_background_music",$data);
		}
