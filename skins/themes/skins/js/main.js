...
App.controller("PageController", function ($scope, $http, $compile) {
	...
	$scope.CreateBlockGroup = function () {
		...
		this.section.init    = function(){
			this.blocks_on   = [];
			this.blocks_off  = [];
			this.onload      = 0;
			this.more        = parseInt(this.ncolum_show_block);
			for (var i = 0; i < this.blocks.length; i++) {
				this.blocks[i].$index = i;
				this.blocks[i].active = 0;
				if (this.blocks[i].id == this.default_block) {
					this.blocks_off.push(this.blocks[i]);
				} else {
					this.blocks_on.push(this.blocks[i]);
				}
			}
		}
		return true;
