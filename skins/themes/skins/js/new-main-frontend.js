...
App.controller("PageController", ($scope, $http, $compile) => {
	...
	$scope.WinDowScreenheight  = $(window).height();
	...
	[
		...
		{
			size : 768,
			label: " > 769px (tablet ngang)",
			active : 0 
		},
		{
			size : 640,
			label: " > 640px (tablet dọc)",
			active : 0 
		},
		{
			size : 540,
	...
	$scope.changeScreen = function(){
		$("#iframe-content").css("height",$("#iframe-content").css("height"));
		$.each($scope.screens,function(){
			this.active = 0;
		});
		if(ContenWidth < $scope.currentScreen.size){
		...
		$scope.reloadPage();
		setTimeout(function(){
			$("#iframe-content").css("height","inherit");
		},200);
		return false;
