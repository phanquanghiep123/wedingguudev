...
(function($)
{
	$.fn.snow = function(options)
	{
		...
		var arg = ["","rotateX","rotateY","rotate","rotateXY","rotateXY"];
		if(options.start == true){
			interval  = setInterval(function()
			{   var key_ram      = 1 * Math.floor(Math.random() * 6);
				...
				$flake.clone().appendTo(options.element).addClass(arg[key_ram]).animate(
					{ left : toleft } ,
					{	duration: delayleft,
						queue: false,
						complete : function(){
							...
							$(this).animate( { left : toleft  },{duration: delayleft,queue: false , complete:function(){
								...
								width_left    = (left - toleft) > 0 ? (left - toleft) : (toleft - left );
								delayleft     = width_left * 10 + Math.random() * 8000;
								key_ram      = 1 * Math.floor(Math.random() * 6);
								$(this).attr("class",arg[key_ram]);
							}})
						}
					}
				).animate(
